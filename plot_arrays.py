...
def read_pos_plot_matrix(ser):
    # Include gains on graph
    gain_read = ser.read_until(b'\n').decode('utf-8')
    gain_read = gain_read.split()
    pgain = float(gain_read[0])
    igain = float(gain_read[1])
    dgain = float(gain_read[2])
    ...
    plt.plot(t,ref,'r*-',t,data,'b*-')
    text_str = 'Proportional Gain = ' + str(pgain) + ' (mA/count)'  + '\nIntegral Gain = ' + str(igain) + ' (mA/(count*s))' + '\nDerivative Gain = '  + str(dgain) + '(mA/(count/s))'
    plt.text(0 , -30, text_str, fontsize = 8, bbox = {'facecolor': 'purple', 'alpha': 0.5, 'pad': 10})
    plt.title('Score = ' + str(score))
    plt.ylabel('Degrees')
    plt.xlabel('Step')
    plt.show()